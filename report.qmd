---
title: |
       Natural convection implementation \
        ```SAMURAI + PETSc```
format: 
    revealjs:
        theme: default
...
*Natural convection in a differentially heated cavity with $\boldsymbol{u}$, $p$, and $T$: being the vector velocity field, pressure and temperature scalar fields.
```{python .hide-code}
...
ax.text(-0.05, 0.5, r'$T=1, \quad \boldsymbol{u}=0, \quad \boldsymbol{\nabla}p = 0$', va='center', ha='right', fontsize=12, rotation=90)         # Left
ax.text(0.5, 1.03, r'$\nabla T=0, \quad \boldsymbol{u}=0, \quad \boldsymbol{\nabla}p = 0$', va='bottom', ha='center', fontsize=12)                     # Top
ax.text(1.05, 0.5, r'$T=0, \quad \boldsymbol{u}=0, \quad \boldsymbol{\nabla}p = 0$', va='center', ha='left', fontsize=12, rotation=90)           # Right
...
ax.text(0.5, -0.03, r"$\nabla T=0, \quad \boldsymbol{u}=0, \quad \boldsymbol{\nabla}p = 0$", va='top', ha='center', fontsize=12)                       # Bottom
...
<span style="font-size: 85%;">*Definition*</span> 
```cpp 
auto conv_temp  = samurai::make_convection_upwind<TemperatureField>(velocity);
```
<span style="font-size: 85%;">*Call*</span> 
```cpp
conv_temp(temperature);
```
<span style="font-size: 85%;">*Implements*</span> 
...
$\quad$ <span style="font-size: 75%;">⚠️ $\ $ It works for incompressible fluid as we explicitly solve for $\nabla \cdot \boldsymbol{u} = 0$ (??).</span> 
...
Parameters</span> -->
- <span style="font-size: 75%;">${\rm Ra} = 1{\rm E+}2$, ${\rm Pr} = 1$, $T_f = 5$</span>
- <span style="font-size: 75%;">$\ell = [6, 4]$, $\epsilon_{\rm MR} = 2{\rm E-4}$, $MR_{\rm regularity} = 1$</span>

![](nat_conv_Ra1e+02.gif){height=400px}

<span style="font-size: 85%;">Remarks</span>

-  <span style="font-size: 85%;">AMR is not well utilised</span>
-  <span style="font-size: 85%;">Ra $>$ 1E+3 : code diverges (explicit buoyancy forcing?, explicit nonlinear terms, finer mesh ?) </span> 
...
<!-- <span style="font-size: 85%;"> 
Parameters</span> -->
- <span style="font-size: 75%;">${\rm Ra} = 1{\rm E+}3$, ${\rm Pr} = 1$, $T_f = 2$</span>
- <span style="font-size: 75%;">$\ell = [6, 4]$, $\epsilon_{\rm MR} = 2{\rm E-4}$, $MR_{\rm regularity} = 1$</span>

![](nat_conv_Ra1e+03.gif){height=400px}
...
## Result: Solid-Fluid CHT with Natural Convection 

**(`SAMURAI` + `PETSc` + <span style="color: red; font-family: monospace;">CWIPI</span>)**

- <span style="font-size: 75%;">${\rm Ra} = 1{\rm E+}2$, ${\rm Pr} = 1$, $T_f = 40$</span>
- <span style="font-size: 75%;">$\ell_{\rm left} = 4$, $\ell_{\rm right} = 5$</span>
- <span style="font-size: 75%;">$\Delta t_{\rm coupling} = 0.1$</span>

![](cht_temp_isosurface.gif){height=400px}

<span style="font-size: 85%;">Remarks</span>

-  <span style="font-size: 85%;">AMR not implemented</span>
-  <span style="font-size: 85%;">First-order BE: slower (probably inefficient implimentation) than ponio </span> 
-  <span style="font-size: 85%;">Ponio: solver implementation with PETSc state vector created by block assembly (possible solution found with JM, not implemented) </span> 
